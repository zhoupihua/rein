...
|-----------|-------|-------------|
| Skills | 21 | Unified skills organized by SDLC phase |
...
| Hooks | 9 | session-start, guard, guard-bash, gate, format, checkbox-guard, leak-guard, inject, artifact-validate |
...
# Global install (to ~/.claude/, all projects share skills/hooks)
...
| `/refactor` | Ralph loop structured refactoring | Smell elimination + commits |
...
| artifact-validate | PostToolUse | Write\|Edit\|MultiEdit | 编辑 changes/ 下工件时验证阶段完整性（Go-only，通过 rein hook 调用） |
...
- **refactor** — Ralph loop: structured refactoring with smell catalog and characterization tests
...
| Skills | 14 + 20 with overlaps | 21 redesigned, no duplicates |
...
| Hooks | Session injection only | 9 hooks (session-start, guard, guard-bash, gate, format, checkbox-guard, leak-guard, inject, artifact-validate) |
